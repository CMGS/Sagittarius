...
def login():
    ...
    if not check:
        return render_template('account.login.html', error=error, login_url=login_url)

    user = get_user_by_email(email=email)
    if not user:
        logger.info('no such user')
        return render_template('account.login.html', error='no such user', login_url=login_url)
    if not user.check_password(password):
        logger.info('invaild passwd')
        return render_template('account.login.html', errorr='invaild passwd', login_url=login_url)
